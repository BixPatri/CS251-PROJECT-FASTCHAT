...
def send_client(status, ID, msg):
    ...
    if status:
        ...
        if prev_msgs==None:
            prev_msgs=[]
        prev_msgs.append(json.dumps(msg))
        # print(prev_msgs)
        curr.execute("""
            UPDATE "Clients"
...
def group_image(message, client_ID):
    curr = db_conn.cursor()
    curr.execute("""
        SELECT "Participants" FROM "Groups" WHERE "ID" = %s
    """,(message["group_id"],))
...
def kick(message, client_ID):
    ...
    message["message"] = str(message["ban_ID"]) + " was KICKED from the group " + str(message["g_ID"]) + "by " + str(client_ID)
...
def Share_group_key(message, sender_ID):
    curr = db_conn.cursor()
    curr.execute("""
        SELECT "ID", "Status" FROM "Clients" WHERE "ID" = %s
    """, (message["Recipient"],))
...
def send_pending(client_ID):
    ...
    if not msgs == None:
        for msg in msgs:
            print(msg)
            send_client(1,client_ID, json.loads(msg))
    curr.close()
...
def handle_client(client, client_ID):
    while True:
        try:
            ...
            elif msg_type == "Share_group_key":
                Share_group_key(message,client_ID)
            elif msg_type == "Send pending msgs":
